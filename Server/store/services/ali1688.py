...
from ..models import (
    Order, OrderItem, OrderReceiver, 
    OrderStatus, RefundStatus, DataPullStatus, DataPullTask
)
...
class Ali1688DataPullService(BaseDataPullService):
    ...
    def _get_trade_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        url_path = self.REQUEST_TYPE_TRADE + 'alibaba.trade.getSellerOrderList/' + self.app_key
    ...
    def _get_single_trade_detail(self, order_id: str) -> Optional[Dict[str, Any]]:
        ...
        url_path = self.REQUEST_TYPE_TRADE + 'alibaba.trade.get.sellerView/' + self.app_key
        _aop_signature = self._calculate_signature(url_path, data)
        data['_aop_signature'] = _aop_signature
        ...
        try:
            order, created = Order.objects.get_or_create(
                ...
                defaults={
                    ...
                    'refund_status': base_info.get('refundStatus', RefundStatus.NO_REFUND),
                    ...
                    'create_time': self._de_format_time(base_info.get('createTime')) if base_info.get('createTime') else None,
                    'pay_time': self._de_format_time(base_info.get('payTime')) if base_info.get('payTime') else None,
                    'modify_time': self._de_format_time(base_info.get('modifyTime')) if base_info.get('modifyTime') else None,
            ...
            if not created:
                ...
                order.refund_status = base_info.get('refundStatus', RefundStatus.NO_REFUND)
                ...
                order.modify_time = self._de_format_time(base_info.get('modifyTime')) if base_info.get('modifyTime') else None
    ...
    def _save_order_items(self, order: Order, product_items: List[Dict[str, Any]]):
        ...
        for item_data in product_items:
            ...
            item, created = OrderItem.objects.get_or_create(
                ...
                defaults={
                    'product_id': str(item_data.get('productID')) if item_data.get('productID') else None,
                    'product_name': item_data.get('name', ''),
                    'product_img_url': ','.join(item_data.get('productImgUrl', [])) if item_data.get('productImgUrl') else None,
                    'product_snapshot_url': item_data.get('productSnapshotUrl'),
                    'product_cargo_number': item_data.get('productCargoNumber') or item_data.get('cargoNumber'),
                    'sku_id': str(item_data.get('skuID')) if item_data.get('skuID') else None,
                    ...
                    'item_refund_status': item_data.get('refundStatus', RefundStatus.NO_REFUND),
                    ...
                    'gmt_create': self._de_format_time(item_data.get('gmtCreate')) if item_data.get('gmtCreate') else None,
                    'gmt_modified': self._de_format_time(item_data.get('gmtModified')) if item_data.get('gmtModified') else None,
            ...
            if not created:
                ...
                item.item_refund_status = item_data.get('refundStatus', RefundStatus.NO_REFUND)
                item.item_amount = item_data.get('itemAmount', 0)
                item.gmt_modified = self._de_format_time(item_data.get('gmtModified')) if item_data.get('gmtModified') else None
                item.platform_raw_data = item_data
                item.save()
        
        if existing_item_ids:
            OrderItem.objects.filter(order=order, platform_item_id__in=existing_item_ids).delete()
