...
def update_product(request, product_id):
    try:
        ...
        # 检查采购成本变更
        if old_purchase_cost != product.purchase_cost:
            price_changes.append({
                'price_type': PriceHistory.PriceType.PURCHASE_COST,
                'old_value': old_purchase_cost,
                'new_value': product.purchase_cost
            })
        
        # 检查成本价格变更
        if old_cost_price != product.cost_price:
            price_changes.append({
                'price_type': PriceHistory.PriceType.COST_PRICE,
                'old_value': old_cost_price,
                'new_value': product.cost_price
            })
        
        # 检查销售价格变更
        if old_selling_price != product.selling_price:
            price_changes.append({
                'price_type': PriceHistory.PriceType.SELLING_PRICE,
                'old_value': old_selling_price,
                'new_value': product.selling_price
            })
        ...
        if product.product_type == ProductType.BEAD:
            try:
                ...
                # 重新计算单颗成本
                old_bead_cost = product.cost_price
                weight = bead.weight
                product.cost_price = product.purchase_cost * weight
                product.save()
                
                # 记录串珠成本变更
                if old_bead_cost != product.cost_price:
                    PriceHistory.objects.create(
                        product=product,
                        price_type=PriceHistory.PriceType.COST_PRICE,
                        old_value=old_bead_cost,
                        new_value=product.cost_price,
                        changed_by=current_user
                    )
            except Bead.DoesNotExist:
            ...
            try:
                ...
                # 重新计算并保存成品的成本价格
                old_finished_cost = product.cost_price
                product.cost_price = calculate_finished_product_cost(finished)
                product.save()
                
                # 记录成品成本变更
                if old_finished_cost != product.cost_price:
                    PriceHistory.objects.create(
                        product=product,
                        price_type=PriceHistory.PriceType.COST_PRICE,
                        old_value=old_finished_cost,
                        new_value=product.cost_price,
                        changed_by=current_user
                    )
            except FinishedProduct.DoesNotExist:
...
def get_price_history(request):
    try:
        ...
        # 构建查询
        price_history_query = PriceHistory.objects.select_related('product', 'changed_by')
        
        # 根据用户权限筛选
        if current_user.user_type not in [UserType.SUPER_ADMIN, UserType.SITE_ADMIN]:
            if current_user.company:
                price_history_query = price_history_query.filter(product__company=current_user.company)
            else:
                price_history_query = PriceHistory.objects.none()
        
        # 按商品类型筛选
        product_type = request.GET.get('product_type')
        if product_type:
            price_history_query = price_history_query.filter(product__product_type=product_type)
        
        # 按商品ID筛选
        product_id = request.GET.get('product_id')
        if product_id:
            price_history_query = price_history_query.filter(product__id=product_id)
        
        # 按价格类型筛选
        price_type = request.GET.get('price_type')
        if price_type:
            price_history_query = price_history_query.filter(price_type=price_type)
        ...
        if start_date:
            try:
                start_dt = datetime.fromisoformat(start_date)
                price_history_query = price_history_query.filter(created_at__gte=start_dt)
            except:
                pass
        ...
        if end_date:
            try:
                end_dt = datetime.fromisoformat(end_date)
                price_history_query = price_history_query.filter(created_at__lte=end_dt)
            except:
                pass
        ...
        # 获取总数
        total_count = price_history_query.count()
        
        # 排序并获取结果
        price_history = price_history_query.order_by('-created_at')[offset:offset + page_size]
        ...
        for record in price_history:
            history_list.append({
                ...
                'product': {
                    ...
                    'product_type_display': record.product.get_product_type_display()
                },
                'price_type': record.price_type,
                'price_type_display': record.get_price_type_display(),
                ...
                'changed_by': {
                    ...
                    'real_name': record.changed_by.real_name
...
@require_http_methods(['GET'])
def get_price_chart_data(request):
    try:
        auth_header = request.headers.get('Authorization')
        if not auth_header or not auth_header.startswith('Bearer '):
            return JsonResponse({'error': '未授权'}, status=401)
        ...
        # 获取参数
        product_id = request.GET.get('product_id')
        price_type = request.GET.get('price_type', PriceHistory.PriceType.COST_PRICE)
        
        if not product_id:
            return JsonResponse({'error': '请指定商品ID'}, status=400)
        
        # 获取商品并检查权限
        product = Product.objects.get(id=product_id)
        if current_user.user_type not in [UserType.SUPER_ADMIN, UserType.SITE_ADMIN]:
            if current_user.company != product.company:
                return JsonResponse({'error': '无权限查看该商品'}, status=403)
        ...
        if not price_history.exists():
            current_price = None
            if price_type == PriceHistory.PriceType.PURCHASE_COST:
                current_price = float(product.purchase_cost)
            elif price_type == PriceHistory.PriceType.COST_PRICE:
                current_price = float(product.cost_price)
            elif price_type == PriceHistory.PriceType.SELLING_PRICE:
                current_price = float(product.selling_price)
            
            if current_price is not None:
                chart_data['dates'].append(product.created_at.date().isoformat())
                chart_data['values'].append(current_price)
        else:
            for record in price_history:
                chart_data['dates'].append(record.created_at.date().isoformat())
                chart_data['values'].append(float(record.new_value))
        ...
        return JsonResponse({
            ...
            'price_type': price_type,
            'price_type_display': PriceHistory(price_type=price_type).get_price_type_display(),
