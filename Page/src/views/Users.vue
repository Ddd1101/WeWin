        ...
        <template #default="{ row }">
          <el-select
            ...
            :disabled="
              ...
              (row.company_is_active !== undefined &&
                row.company_is_active !== null &&
...
const filteredUsers = computed(() => {
  return users.value.filter((user) => {
    ...
    if (
      ...
      !user.username
        .toLowerCase()
        .includes(filterForm.value.username.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      ...
      user.user_type !== filterForm.value.userType
    ) {
      return false;
    }
    ...
    if (filterForm.value.company) {
      ...
      if (
        !companyName.includes(companyFilter) &&
        !companyCode.includes(companyFilter)
      ) {
        return false;
      }
    }
...
const changeUserType = async (user) => {
  try {
    ...
    const originalUserType = originalUser
      ? originalUser.user_type
      : user.user_type;
    const newUserType = user.user_type;

    const token = localStorage.getItem("token");
    const response = await fetch(
      `${API_BASE_URL}/api/account/users/${user.id}/update-type/`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
...
const toggleUserStatus = async (user) => {
  try {
    // 从原始用户列表中获取当前用户的原始状态
    const originalUser = users.value.find((u) => u.id === user.id);
    const originalStatus = originalUser
    ...
    const token = localStorage.getItem("token");
    const response = await fetch(
      `${API_BASE_URL}/api/account/users/${user.id}/status/`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
...
const batchUpdateUsers = async (isActive, actionText) => {
  try {
    ...
    // 过滤掉超级管理员
    const usersToUpdate = selectedUsers.value.filter(
      (u) => u.user_type !== "super_admin",
    );

    if (usersToUpdate.length === 0) {
      ElMessage.warning(
        `没有可${actionText}的用户（超级管理员不能被批量操作）`,
      );
      return;
    }

    await ElMessageBox.confirm(
      `确定要批量${actionText}选中的 ${usersToUpdate.length} 个用户吗？`,
      "提示",
      {
    ...
    if (successCount > 0) {
      ElMessage.success(
        `批量${actionText}成功 ${successCount} 个用户${failCount > 0 ? `，${failCount} 个用户${actionText}失败` : ""}`,
      );
    }
